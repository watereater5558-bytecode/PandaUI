...
return function(Config)
	...
	if not RunService:IsStudio() and Window.Folder and writefile then
		if not isfolder("PandaUI/" .. Window.Folder) then
			makefolder("PandaUI/" .. Window.Folder)
		end
		if not isfolder("PandaUI/" .. Window.Folder .. "/assets") then
			makefolder("PandaUI/" .. Window.Folder .. "/assets")
		end
	...
	if typeof(Window.Background) == "string" and BGVideo then
		...
		if string.find(BGVideo, "http") then
			...
			if not isfile(videoPath) then
				...
				if not success then
					warn("[ PandaUI.Window.Background ] Failed to download video: " .. tostring(result))
					return
				end
			end
			...
			if not success then
				warn("[ PandaUI.Window.Background ] Failed to load custom asset: " .. tostring(customAsset))
				return
			end
	...
	Creator.AddSignal(Window.UIElements.Main.Main.Topbar.Left:GetPropertyChangedSignal("AbsoluteSize"), function()
		...
		local RightWidth = Window.UIElements.Main.Main.Topbar.Right.UIListLayout.AbsoluteContentSize.X
			/ Config.PandaUI.UIScale
		LeftWidth = Window.UIElements.Main.Main.Topbar.Left.AbsoluteSize.X / Config.PandaUI.UIScale
		...
		Window.UIElements.Main.Main.Topbar.Center.Position =
			UDim2.new(0, LeftWidth + (Window.UIPadding / Config.PandaUI.UIScale), 0.5, 0)
		Window.UIElements.Main.Main.Topbar.Center.Size =
			UDim2.new(1, -LeftWidth - RightWidth - ((Window.UIPadding * 2) / Config.PandaUI.UIScale), 1, 0)
	end)
	if Window.Topbar.ButtonsType ~= "Default" then
		Creator.AddSignal(Window.UIElements.Main.Main.Topbar.Right:GetPropertyChangedSignal("AbsoluteSize"), function()
			Window.UIElements.Main.Main.Topbar.Left.Position = UDim2.new(
				0,
				(Window.UIElements.Main.Main.Topbar.Right.AbsoluteSize.X / Config.PandaUI.UIScale) + Window.UIPadding - 4,
				0,
				0
			)
		end)
	...
	if not IsVideoBG and Window.Background and typeof(Window.Background) == "table" then
		...
		Window.UIElements.BackgroundGradient = Creator.NewRoundFrame(Window.UICorner, "Squircle", {
			...
			ImageTransparency = Window.Transparent and Config.PandaUI.TransparencyValue or 0,
		}, {
			BackgroundGradient,
		})
	end
	...
	function Window:SetBackgroundTransparency(v)
		...
		Config.PandaUI.TransparencyValue = rounded
	...
	if Config.PandaUI.UseAcrylic then
	...
	function Window:Open()
		task.spawn(function()
			...
			Config.PandaUI.UIScaleObj.Scale -= 1 - 0.85
			Tween(
				Config.PandaUI.UIScaleObj,
				0.33,
				{ Scale = Config.PandaUI.UIScale },
			...
			task.spawn(function()
				...
				Config.PandaUI:ToggleAcrylic(true)
			end)
		end)
	end
	function Window:Close()
		...
		Config.PandaUI:ToggleAcrylic(false)
		...
		Tween(
			Config.PandaUI.UIScaleObj,
			0.28,
			{ Scale = Config.PandaUI.UIScale - (1 - 0.85) },
			Enum.EasingStyle.Quint,
			Enum.EasingDirection.Out
		):Play()
		if BGImage then
		...
		function Close:Destroy()
			task.spawn(function()
				...
				task.wait(0.4)
				Config.PandaUI.ScreenGui:Destroy()
				Config.PandaUI.NotificationGui:Destroy()
				Config.PandaUI.DropdownGui:Destroy()
				Config.PandaUI.TooltipGui:Destroy()
	...
	function Window:ToggleTransparency(Value)
		...
		Config.PandaUI.Transparent = Value
		Window.UIElements.Main.Background.ImageTransparency = Value and Config.PandaUI.TransparencyValue or 0
	end
	...
	function Window:GetUIScale(v)
		return Config.PandaUI.UIScale
	end
	function Window:SetUIScale(v)
		Config.PandaUI.UIScale = v
		Tween(Config.PandaUI.UIScaleObj, 0.2, { Scale = v }, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
		return Window
	end
	...
	local TabModule = TabModuleMain.Init(Window, Config.PandaUI, Config.PandaUI.TooltipGui)
	...
	function Window:Tab(TabConfig)
		...
		return TabModule.New(TabConfig, Config.PandaUI.UIScale)
	end
	...
	function Window:Dialog(DialogConfig)
		...
		local Dialog = DialogModule.Create(false, "Dialog", Window, Config.PandaUI, Window.UIElements.Main.Main)
		...
		local function CheckButtonsOverflow()
			...
			wait()
			local totalWidth = ButtonsLayout.AbsoluteContentSize.X / Config.PandaUI.UIScale
			local parentWidth = ButtonsContent.AbsoluteSize.X / Config.PandaUI.UIScale
			if totalWidth > parentWidth then
				...
				if availableSpace > 0 then
					...
					for _, button in ipairs(Buttons) do
						local buttonWidth = button.AbsoluteSize.X / Config.PandaUI.UIScale
