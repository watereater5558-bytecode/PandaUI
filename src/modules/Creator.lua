...
function Creator.Init(PandaUITable)
	PandaUI = PandaUITable
...
function Creator.SafeCallback(Function, ...)
	...
	if not Success then
		if PandaUI and PandaUI.Window and PandaUI.Window.Debug then
			local _, i = Event:find(":%d+: ")
			warn("[ PandaUI: DEBUG Mode ] " .. Event)
			return PandaUI:Notify({
...
function Creator.Gradient(stops, props)
	if PandaUI and PandaUI.Gradient then
		return PandaUI:Gradient(stops, props)
	end
	local colorSequence = {}
...
function Creator.Image(Img, Name, Corner, Folder, Type, IsThemeTag, Themed, ThemeTagName)
	...
	elseif string.find(Img, "http") and not string.find(Img, "roblox.com") then
		local FileName = "PandaUI/" .. Folder .. "/assets/." .. Type .. "-" .. Name .. ".png"
